    ...
    <div class="container paddingWithoutNav">
        ...
        <div class="col-md-3">
                ...
                if (isset($_SESSION['estado']) && $_SESSION['estado'] == "online") {
                    ...
                    }else if ($_SESSION['tipo'] == "admin" || $_SESSION['id'] == $tuplaPro['idUsuario'])  {
                         ?>
                         <p class="lead">Ofertas</p>
                         <?php
                         if($cantOfe2==0){
                            ?>
                         <p >No recibio ofertas</p>
                         <?php
                         }
                         else{
                         while ($tuplaOfe = mysqli_fetch_array($resOfe2)) {
                             if(interval_date($fechaActual[0],$tuplaPro['fecha_fin'])==="Publicaci&oacute;n finalizada."){
                              ?>
                                 <a type='button' id='elegirGanador_<?php echo $tuplaOfe['idUsuario'];?>' class='btn btn-primary elegirGanador' data-toggle='modal' data-target='.modalGanador'> <?php echo  $tuplaOfe['oferta'];?></a>
                            ...
                            ?>
                             <li><?php echo $tuplaOfe['oferta'];?></li>
                            <?php
                            }
                         }
                     }
...
$(".elegirGanador").click(function() {
            var values = $(this).attr('id').split("_"),
                idG = parseInt(values[1]);
                console.log(idG);
            $.post('seleccionarGanador.php',{idGanador:idG},function(response){
                $('#buttonModalLinkGanador').html(response);
            });
        });
</script>
<!--MODAL SELECCIONAR GANADOR-->
            <div class="modal fade modalGanador" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel">
                                    <div class="modal-dialog modal-sm">
                                        <div class="modal-content">
                                            <div class="modal-header">
                                            <span>Desea elegir la oferta como ganadora?</span>
                                            </div>
                                            <div class="modal-footer">
                                                <button type="button" class="btn btn-default" data-dismiss="modal">No</button>
                                                <a id="buttonModalLinkGanador"></a>
                                            </div>
                                        </div>
                                      </div>
                                    </div>
